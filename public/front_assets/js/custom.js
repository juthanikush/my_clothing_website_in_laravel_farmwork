...
function change_product_color_image(img,color){
  ...
  jQuery('.simpleLens-big-image-container').html('<a data-lens-image="'+img+'" class="simpleLens-lens-image"><img src="'+img+'" class="simpleLens-big-image"><div class="simpleLens-mouse-cursor"></div></a>')
}
      ...
      success:function(result){
        ...
        if(result.msg=='not_avaliable'){
          ...
          if(result.totalItem==0){
           ...
           var html='<ul>';
           jQuery.each(result.data['list'], function(arrKey,arrVal){
...
jQuery('#frmRegistrashion').submit(function(e){
  e.preventDefault();
  jQuery('.field_error').html('');
  jQuery.ajax({
    url:'registrashion_process',
    data:jQuery('#frmRegistrashion').serialize(),
    type:'post',
    success:function(result){
      ...
      if(result.status=="success"){
        jQuery('#frmRegistrashion')[0].reset();
...
jQuery('#frmForgot').submit(function(e){
  ...
  jQuery('#forget_msg').html("Pleace wait...");
  e.preventDefault();
  ...
  jQuery.ajax({
    ...
    type:'post',
    success:function(result){
      jQuery('#forget_msg').html(result.msg);
    }
  });
});

jQuery('#frmUpdatePassword').submit(function(e){
  jQuery('#thank_you_msg').html("Pleace wait...");
...
function applyCouponcode(){
  ...
  if(coupon_code!='')
  {
    ...
    jQuery('#coupon_code_msg').html('Place Enter Coupon code');
  }
}
...
jQuery('#frmPlaceOrder').submit(function(e){
  jQuery('#order_place_msg').html("Pleace wait...");
